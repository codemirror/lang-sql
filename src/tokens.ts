...
function append(result: {[name: string]: number}, keyword: string, Type: any, log: string) {
  if (!keyword) return
  if (result[keyword] && result[keyword] != Type)  {
    console.warn(`Duplicated keyword '${keyword}'. Overwriting with a ${log}`);
  }
...
function keywords(keywords: string, types: string, builtin?: string) {
  ...
  for (let kw of keywords.split(" ")) append(result, kw, Keyword, 'Keyword')
  for (let tp of types.split(" ")) append(result, tp, Type, 'Type')
  for (let kw of (builtin || "").split(" ")) append(result, kw, Builtin, 'Builtin')
  return result
}
